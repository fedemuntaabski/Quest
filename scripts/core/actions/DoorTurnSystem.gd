...
## ticks resource production, and emits a placeholder enemy-wave signal
## (no real enemy system exists to consume it yet).
...
func open_room(room_id: String) -> bool:
	...
	QuestLogger.info(QuestLogger.Category.DOOR, "Enemy wave requested for room '%s' (placeholder, no EnemyManager)." % room_id)
