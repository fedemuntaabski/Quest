...
static func process_enemy_defeat(manager: EnemyManager, enemy, room_id: int) -> void:
	...
	QuestLogger.info(QuestLogger.Category.COMBAT, "Enemy defeated: +%dg (accumulated total: %dg)" % [reward_gold, manager._run_accumulated_gold])
...
static func grant_and_reset_accumulated_gold(manager: EnemyManager) -> int:
	...
	QuestLogger.info(QuestLogger.Category.SAVE, "Run ended: granted accumulated gold +%dg" % amount)
	manager._run_accumulated_gold = 0
	return amount
