...
func save_game(slot: int = current_slot) -> void:
	...
	if player_stats_autoload and player_stats_autoload.stats:
		var st = player_stats_autoload.stats
		cfg.set_value(SAVE_SECTION, "max_hp", st.max_hp)
		cfg.set_value(SAVE_SECTION, "strength_modifier", st.strength_modifier)
		cfg.set_value(SAVE_SECTION, "magic_modifier", st.magic_modifier)
		cfg.set_value(SAVE_SECTION, "dexterity_modifier", st.dexterity_modifier)
		cfg.set_value(SAVE_SECTION, "active_upgrades", player_stats_autoload.active_upgrades)
	else:
		cfg.set_value(SAVE_SECTION, "max_hp", 10)
		cfg.set_value(SAVE_SECTION, "strength_modifier", 0)
		cfg.set_value(SAVE_SECTION, "magic_modifier", 0)
		cfg.set_value(SAVE_SECTION, "dexterity_modifier", 0)
...
func load_game(slot: int = current_slot) -> void:
	...
	if err == OK:
		...
		if player_stats_autoload:
			player_stats_autoload.base_hp = cfg.get_value(SAVE_SECTION, "max_hp", 10)
			player_stats_autoload.base_str = cfg.get_value(SAVE_SECTION, "strength_modifier", 0)
			player_stats_autoload.base_mag = cfg.get_value(SAVE_SECTION, "magic_modifier", 0)
			player_stats_autoload.base_dex = cfg.get_value(SAVE_SECTION, "dexterity_modifier", 0)
