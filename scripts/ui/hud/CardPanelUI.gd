...
func _create_card_row(slot_num: int, data: Dictionary) -> void:
	...
	var tex: Texture2D = view.get("icon", null)
	...
	name_label.text = view.get("display_name", "Unknown Card")
	name_label.add_theme_color_override("font_color", Color(1, 1, 1, 1))
	name_hbox.add_child(name_label)
	
	var cd_remaining: int = int(view.get("cooldown_remaining", 0))
	if cd_remaining > 0:
	...
	var state := str(view.get("state", "available")).to_upper()
	...
	stats_label.text = view.get("stats_summary", "")
