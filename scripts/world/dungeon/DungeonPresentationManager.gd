...
func build(tileset: TileSet, wall_texture: Texture2D) -> void:
	...
	for edge in graph.get_edge_records_sorted():
		var corridor_node := generator.room_factory.create_corridor_entity(edge, generator.corridors_root)
		graph.set_edge_runtime_node(edge["room_a"], edge["room_b"], corridor_node)

	map_renderer.setup(generator, tileset, generator.grid_origin, generator.rng)
